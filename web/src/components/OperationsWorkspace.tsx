...
function upstreamDiagnosticPhrase(sourceHttpStatus?: number, sourceRequestId?: string, sourceMatchedPath?: string, sourceCacheStatus?: string, sourceBodyEmpty?: boolean, sourceContentType?: string, sourceContentTypeMissing?: boolean) {
  ...
  const parts = [sourceHttpStatus ? `upstream HTTP ${sourceHttpStatus}` : null, sourceRequestId ? `request ${sourceRequestId}` : null, matchedPathPart, cachePart, sourceBodyEmpty ? "empty upstream body" : null, contentTypePart].filter(Boolean);
...
type SourceState =
  ...
  | { status: "error"; campaignId: string; title: string; message: string; runStatus?: RunReadModel["status"]; sourceOrigin?: string; sourceStep?: SourceFailureStep; retryAfter?: string; sourceHttpStatus?: number; sourceRequestId?: string; sourceMatchedPath?: string; sourceCacheStatus?: string; sourceBodyEmpty?: boolean; sourceContentType?: string; sourceContentTypeMissing?: boolean; checkedAt?: string }
...
async function fetchCampaignSource(campaignId: string, signal: AbortSignal): Promise<CampaignSource> {
  ...
  if (!sourceRes.ok) {
    ...
    const errorBody = sourceBody as { error?: string; detail?: string; runStatus?: RunReadModel["status"]; sourceOrigin?: string; sourceStep?: unknown; sourceHttpStatus?: unknown; sourceRequestId?: unknown; sourceMatchedPath?: unknown; sourceCacheStatus?: unknown; sourceBodyEmpty?: unknown; sourceContentType?: unknown; sourceContentTypeMissing?: unknown } | null;
              ...
              <article key={item.campaign.id} className={`rounded-[var(--r-2xl)] border p-4 shadow-sm ${item.campaign.conferenceHero ? "border-ops-ink bg-ops-yellow/50" : "border-ops-line bg-background"}`}>
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_260px] lg:items-center">
                  <div>
                    ...
                    {item.status === "error" && upstreamDiagnosticPhrase(item.sourceHttpStatus, item.sourceRequestId, item.sourceMatchedPath, item.sourceCacheStatus, item.sourceBodyEmpty, item.sourceContentType, item.sourceContentTypeMissing) ? (
                      <p className="mt-2 text-xs text-muted-foreground">
                        Source response: <span className="font-medium text-foreground">{upstreamDiagnosticPhrase(item.sourceHttpStatus, item.sourceRequestId, item.sourceMatchedPath, item.sourceCacheStatus, item.sourceBodyEmpty, item.sourceContentType, item.sourceContentTypeMissing)}</span>
                      </p>
                    ) : null}
...
function SourceStateShell({ state, onRetry }: { state: Exclude<SourceState, { status: "fixture" } | { status: "ready" }>; onRetry?: () => void }) {
  ...
  const sourceDiagnostic = "sourceHttpStatus" in state ? upstreamDiagnosticPhrase(state.sourceHttpStatus, state.sourceRequestId, state.sourceMatchedPath, state.sourceCacheStatus, state.sourceBodyEmpty, state.sourceContentType, state.sourceContentTypeMissing) : null;
    ...
    fetchCampaignSource(campaignId, controller.signal)
      ...
      .catch((error: unknown) => {
        ...
        if (runStatus && runStatus !== "completed" && runStatus !== "partial") {
          setSourceState({ status: "unavailable", campaignId, title: "Campaign not usable yet", message, runStatus, sourceOrigin, sourceStep, retryAfter, sourceHttpStatus, sourceRequestId, sourceMatchedPath, sourceCacheStatus, sourceBodyEmpty, sourceContentType, sourceContentTypeMissing, checkedAt: new Date().toISOString() });
          return;
        }
        setSourceState({ status: "error", campaignId, title: "Campaign source unavailable", message, runStatus, sourceOrigin, sourceStep, retryAfter, sourceHttpStatus, sourceRequestId, sourceMatchedPath, sourceCacheStatus, sourceBodyEmpty, sourceContentType, sourceContentTypeMissing, checkedAt: new Date().toISOString() });
      });
      ...
      fetchCampaignSource(campaign.id, controller.signal)
        ...
        .catch((error: unknown) => {
          ...
          setSwitcherItems((current) =>
            current.map((item) =>
              item.campaign.id === campaign.id
                ? { campaign, status: "error", message, runStatus, sourceOrigin, sourceStep, retryAfter, sourceHttpStatus, sourceRequestId, sourceMatchedPath, sourceCacheStatus, sourceBodyEmpty, sourceContentType, sourceContentTypeMissing, checkedAt: new Date().toISOString() }
                : item,
            ),
          );
        });
                ...
                {switcherItems.map((item) => {
                  ...
                  const failureDetails = item.status === "error"
                    ? [
                        ...
                        upstreamDiagnosticPhrase(item.sourceHttpStatus, item.sourceRequestId, item.sourceMatchedPath, item.sourceCacheStatus, item.sourceBodyEmpty, item.sourceContentType, item.sourceContentTypeMissing),
