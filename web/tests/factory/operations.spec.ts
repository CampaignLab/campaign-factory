...
test("operations source API: empty upstream run failures stay source-unavailable before document hydration", async () => {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (String(input).endsWith(`/api/factory/runs/${curatedId}`)) {
      return new Response(null, { status: 500, headers: { "content-length": "0", "x-matched-path": "/api/factory/runs/[id]", "x-vercel-cache": "MISS", "x-vercel-id": "lhr1::iad1::ops-500" } });
    }
  ...
  try {
    ...
    const body = (await response.json()) as { error?: string; detail?: string; sourceOrigin?: string; sourceStep?: string; sourceHttpStatus?: number; sourceRequestId?: string; sourceMatchedPath?: string; sourceCacheStatus?: string; sourceBodyEmpty?: boolean; sourceContentTypeMissing?: boolean; documents?: unknown[]; sourceRunUnavailable?: boolean };
    expect(body.error).toBe("Campaign source run unavailable");
...
test("operations workspace: failed direct source load keeps canonical source brief links", async ({ page }) => {
  ...
  await expect(page.getByText("Source response:")).toBeVisible();
  await expect(page.getByText("upstream HTTP 500 · request lhr1::iad1::direct-500 · matched /api/factory/runs/[id]/documents · cache MISS · empty upstream body · no upstream content type")).toBeVisible();
