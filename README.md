...
The rewrite replaces the four-call pipeline with a real **fifteen-agent campaign graph** (13 fixed responsibilities plus 2 campaign-selected specialists; 20 hard cap). It is the same product idea and the same integrity principle, expressed as a durable multi-agent runtime rather than a routed pipeline. It ships as **two surfaces** over one graph:

- **Public single campaign** at **[`/factory`](web/src/app/factory/page.tsx)** — enter a problem + place and watch one campaign assemble live in the **Campaign Assembly View** (Agent Work Cards, evidence states, progressive ten-step Campaign Brief, nine canonical documents, Campaign Completion Receipt).
- **Presenter-coded batches** at **[`/factory/present`](web/src/app/factory/present/page.tsx)** — a code-gated presenter enters 1–5 campaigns that fan out concurrently into the **Factory Gallery** (`/factory/gallery/[batchId]`), with a Batch Receipt and an immutable replay pinned at `/factory/replay/conference`.
